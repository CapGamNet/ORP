...
if (!isNull life_sitting) exitWith { hint "You are already sitting!" };
if (vehicle _unit != _unit) exitWith { hint "You are already sitting here!!" };
if (_unit distance _chair > 4) exitWith { hint "I hope your feet are so long?" };
if (_unit getVariable ["restrained",false]) exitWith {};
if (_unit getVariable ["isTazed",false]) exitWith {};

if (!isNull (_chair getVariable ["sitting", objNull])) exitWith { hint "Do you think someone is sitting on top of you ?!"; };
...
_action = _unit addAction ["<t color='#B45F04'>Stand up</t>","scripts\standup.sqf"];
