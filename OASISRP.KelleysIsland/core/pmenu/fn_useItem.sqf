...
private _profileName = _player getVariable ["realname",name _player];
...
switch (true) do {
    ...
    case (_item in ["vodka"]): {
        if ([false,_item,1] call life_fnc_handleInv) then {
            [_item, _uid] call TFGG_fnc_useAlcohol;
        };
    };
    ...
    case (_item isEqualTo "portableCam"): {
        if (!isNull life_portableCamera) exitWith {closeDialog 0};
        {life_portableCameras = life_portableCameras - [objNull]} forEach life_portableCameras;
        _cameraLimit = getnumber(missionConfigFile >> "camera_system_config" >> "general" >> "cameraLimit");
        _cameraLimitText = getText(missionConfigFile >> "camera_system_config" >> "general" >> "cameraLimitText");
        if (count life_portableCameras > _cameraLimit) exitWith {hint _cameraLimitText};
        if ([false,_item,1] call life_fnc_handleInv) then {
            [] spawn mav_camera_fnc_placeCamera;
            closeDialog 0;
        };
    };
