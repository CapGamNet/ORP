...
if (_mode) then {
    _purchasePrice = ((round(_initalPrice * _buyMultiplier)) * _devideTax);
} else {
    _purchasePrice = ((round(_initalPrice * _rentMultiplier)) * _devideTax);
};
private _taxPaid =  (_purchasePrice - (round(_initalPrice * _buyMultiplier)));
...
hint parseText format [localize "STR_Shop_Veh_Bought",getText(configFile >> "CfgVehicles" >> _className >> "displayName"),[_purchasePrice] call life_fnc_numberText, [_taxPaid] call life_fnc_numberText];
