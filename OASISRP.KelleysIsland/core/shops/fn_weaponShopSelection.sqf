...
if ((uiNamespace getVariable ["Weapon_Shop_Filter",0]) isEqualTo 1) then {
    ...
    _priceTag ctrlSetStructuredText parseText format ["<t size='0.8'>Price: <t color='#8cff9b'>$%1</t></t>",[(_priceWithTax)] call life_fnc_numberText];
    _control lbSetValue[_index,_price];
} else {
    _price = _control lbValue _index;
    _getTax = missionNamespace getVariable "tax";//should return the number from the databse
    _devideTax = _getTax / 100;
    _priceWithTax = _price * _devideTax;
    _item = CONTROL_DATAI(_control,_index);
    if (_priceWithTax > CASH) then {
        _priceTag ctrlSetStructuredText parseText format ["<t size='0.8'>Price: <t color='#ff0000'>$%1</t><br/>You lack: <t color='#8cff9b'>$%2</t></t>",[(_priceWithTax)] call life_fnc_numberText,[(_priceWithTax - CASH)] call life_fnc_numberText];
    } else {
        _priceTag ctrlSetStructuredText parseText format ["<t size='0.8'>Price: <t color='#8cff9b'>$%1</t></t>",[(_priceWithTax)] call life_fnc_numberText];
    };
