...
if (!isNull life_portable_cam) exitWith {
    [_code] call mav_camera_fnc_keyHandlerCamera;
    _handled;
};
...
switch (_code) do {
    ...
    _viewCameraKey = getnumber(missionConfigFile >> "camera_system_config" >> "keybindings" >> "viewCameraKey");
    _requireShift = getnumber(missionConfigFile >> "camera_system_config" >> "keybindings" >> "requireShift");
    _requireControl = getnumber(missionConfigFile >> "camera_system_config" >> "keybindings" >> "requireControl");
    _requireAlt = getnumber(missionConfigFile >> "camera_system_config" >> "keybindings" >> "requireAlt");

    case _viewCameraKey: {
        if ((_requireShift && !_isShift) || {(_requireControl && !_ctrlKey)} || {(_requireAlt && !_alt)}) exitWith {};
        [0] call mav_camera_fnc_activateCamera;
    };
    ...
    /*case 38: {
        ...
        if ((!_alt && !_ctrlKey) && (side player == west)) then { [] call life_fnc_radar; };
    };*/

    //Y Player Menu
    case 21: {
        if (!_alt && !_ctrlKey && !dialog && !(player getVariable ["restrained",false]) && {!life_action_inUse} && !(player getVariable ["ziptied",false])) then {
    ...
    case 33: {
        if (playerSide in [west,independent] && {vehicle player != player} && {!life_siren_active} && {((driver vehicle player) == player)}) then {
            ...
            if ((_veh getVariable "siren")) then {
                ["Sirens Off","You have turned off your sirens.",[1,0,0,1]] call Lega_fnc_addNotification;
                ...
                ["Sirens On","You have turned on your sirens.",[0,1,0,1]] call Lega_fnc_addNotification;
