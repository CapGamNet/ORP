...
if ((((_curObject getVariable "house_owner") select 0) == (getPlayerUID player)) && (_curObject isKindOf "House_F") && ({player distance _curObject < 12})) exitWith {
    [_curObject] call life_fnc_houseSellMenu;  
};
if ((_curObject isKindOf "House_F") && ({player distance _curObject < 12})) exitWith {[_curObject] call life_fnc_houseBuyMenu;};
...
if (isPlayer _curObject && _curObject isKindOf "Man") then {
    ...
    _miscItems = ["Land_BottlePlastic_V1_F","Land_TacticalBacon_F","Land_Can_V3_F","Land_CanisterFuel_F","Land_Suitcase_F","Land_Defibrillator_F"];
...
_distanceCorn = position player distance getMarkerPos "corn_mark";
_distanceWheat = position player distance getMarkerPos "wheat_mark";
_distanceWeed = position player distance getMarkerPos "weed_mark";
    {
    if (_x < 30) exitWith {[] call Lega_fnc_openplantmenu;};        
    } forEach [_distanceCorn,_distanceWeed,_distanceWheat];
